...
def parse_chat(text: str):
    ...
    for i, m in enumerate(matches):
        ...
        # Dispositivos: se reportan como "(N)" en la reseña. Sumamos todos.
        # Ej: "cinco (05) teléfonos celulares, un (1) pendrive, una (1) laptop" → 5+1+1 = 7
        rm = re.search(r'\*?rese[ñn]a\*?[:\*]*\s*([^\n\r]+)', body, re.IGNORECASE)
        dispositivos = 0
        if rm:
            dispositivos = sum(int(n) for n in re.findall(r'\((\d+)\)', rm.group(1)))
...
def main():
    ...
    print(f'  Eventos totales : {len(events)}  ({total_disp} dispositivos)')
    print(f'  Inicios         : {by_tipo.get("inicio", 0)}  ({disp_by_tipo.get("inicio", 0)} disp.)')
    print(f'  Finalizaciones  : {by_tipo.get("finalizacion", 0)}  ({disp_by_tipo.get("finalizacion", 0)} disp.)')
    print(f'  Recepciones     : {by_tipo.get("recepcion", 0)}  ({disp_by_tipo.get("recepcion", 0)} disp.)')
